...
class TabluePoc1 extends Component {
    ...
    onAccClick = async(accId) => {
        await axios.get("http://34.122.239.255:32599/getsummary/"+accId).then(response => {
                                    ...
                                    <Table hover className="complianceStandardTable">
                                        ...
                                        <tbody>
                                            {this.state.accList && this.state.accList.map((item, i) =>
                                                <tr key={i}>
                                                    <td><NavLink className="linkStyle" onClick={()=>this.onAccClick(item.account_id)}>{this._dataContextUtil.toUpperCase(item.name)} <FontAwesomeIcon className="fs12" icon={faExternalLinkAlt} /></NavLink></td>
                                                </tr>
