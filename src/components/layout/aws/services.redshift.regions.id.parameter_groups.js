...
import { Breadcrumb, BreadcrumbItem, Nav, NavItem, NavLink, Row, ListGroup, ListGroupItem, Col } from 'reactstrap';
...
export default class RedshiftRegionParameterGroup extends Component {
                                ...
                                this.state.data.map((item, i) =>
                                    <NavItem key={i}>
                                        ...
                                        <Nav vertical className="bgWhite ml-2">
                                            {
                                                Object.entries(item[1].parameter_groups).map((parameter_groups, j) =>
                                                    <NavItem key={j}>
                                                        <NavLink className={'innerA ' +
                                                            (this.state.displaybleData && this.state.displaybleData.includes(parameter_groups[0]) ? "d-block" : "d-none")
                                                            // (this.state.showThis === item[1].name ? "active " : null)
                                                        }
                                                            onClick={() => {
                                                                this.toggle(parameter_groups[0] + '_' + i + '_' + j);
                                                            }}>
                                                            {parameter_groups[1].name}
                                            ...
                                            >
                                                 ...
                                                 <AccordionSummary
                                                    ...
                                                    >
                                                        ...
                                                        <h5>Information</h5>
                                                        <ListGroup key={i}>
                                                        <ListGroupItem className="pb-2 font-weight-500">Description:{parameter_groups[1].description}</ListGroupItem>
                                                        <ListGroupItem className="pb-2 font-weight-500">Group Family:{parameter_groups[1].is_default ? "True" : "False"}</ListGroupItem>
                                                        </ListGroup>
                                                    </div>
                                                    <hr />
                                                    <div className="p-3">
                                                        <h5>Parameters</h5>
                                                        {
                                                            parameter_groups[1].parameters_count > 0 && Object.entries(parameter_groups[1].parameters).map(([key, val]) =>
                                                            <ListGroup key={i}>
                                                                    <ListGroupItem><span className="pb-2 font-weight-500">{key}: </span>{val.value}</ListGroupItem>
                                                            </ListGroup>
                                                            )
